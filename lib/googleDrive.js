...
// Lists photos tagged for one property. Files given an explicit manual order
// (a numeric `sortOrder` appProperty, written by reorderPhotos) come first in
// that order; any others keep the default newest-first order. So the gallery
// reads newest-first until the user reorders, then honors the custom order —
// and newly uploaded photos (no sortOrder yet) land after the ordered ones.
// Excludes the property folder itself, which carries the same tags.
export async function listPhotos(dealType, id) {
  ...
  const hasOrder = (f) =>
    f.appProperties && f.appProperties.sortOrder !== undefined && f.appProperties.sortOrder !== "";
  const ordered = files
    .filter(hasOrder)
    .sort((a, b) => Number(a.appProperties.sortOrder) - Number(b.appProperties.sortOrder));
  const rest = files.filter((f) => !hasOrder(f));
  return [...ordered, ...rest].map(({ id, name, createdTime }) => ({ id, name, createdTime }));
}

// Persists a manual photo order by writing a numeric `sortOrder` appProperty
// (the file's index in `fileIds`) onto each file. listPhotos then returns them
// in that order. Only files this app created (drive.file scope) are updatable.
export async function reorderPhotos(fileIds) {
  const drive = getDrive();
  ...
  await Promise.all(
    fileIds.map((fileId, i) =>
      drive.files.update({
        fileId,
        requestBody: { appProperties: { sortOrder: String(i) } },
      })
    )
  );
  return true;
