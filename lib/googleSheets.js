...
export async function writePurchasedFlag(sheetName, id, purchased) {
  ...
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth) {
    console.warn("writePurchasedFlag: no service account credentials configured");
    return false;
  }
  if (!GOOGLE_SHEET_ID) {
...
export async function testEditorAccess() {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth) return { attempted: false, reason: "no service account credentials configured" };
...
export async function diagnoseSheet(sheetName) {
  ...
  if (!readAuth || !GOOGLE_SHEET_ID) {
    result.error = "Missing service account credentials or GOOGLE_SHEET_ID env var";
    return result;
  }
