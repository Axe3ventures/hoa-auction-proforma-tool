...
// Purchaser name, sale price, and purchase date live at these FIXED columns
// on every sheet tab, independent of header text — deliberately not looked up
// by header name (unlike the ID column) so this doesn't depend on any
// particular header label existing at all.
// AB/AC were already in use on the user's live sheet (Drive-By Notes / Deal
// Notes, shifted right by the manually-inserted "Purchased" column at B) —
// AD/AE/AF are the actual first empty columns, confirmed empty across all
// rows before use.
const PURCHASER_COL = "AD";
const PRICE_COL = "AE";
const DATE_COL = "AF";
...
// Reads columns AD (purchaser), AE (price), and AF (purchase date) for each
// data row (row 2 onward), aligned with the data rows the same way
...
export async function fetchPurchaseInfo(sheetName) {
  ...
  try {
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: `${sheetName}!${PURCHASER_COL}2:${DATE_COL}1000`,
    });
    const rows = res.data.values || [];
    return rows.map((row) => ({ purchaser: row[0] || "", price: row[1] || "", purchasedDate: row[2] || "" }));
  } catch (err) {
    // Columns AD:AF don't exist yet on a narrower tab (grid isn't wide
...
// Writes the purchaser name, sale price, and purchase date into columns
// AD/AE/AF for the row matching `id` (still looked up by the ID column,
// wherever that is), so purchase details survive across machines/deploys
// instead of living only in a local file. Pass empty strings to clear all
// three (undoing a purchase). Returns false (caller should fall back to local
// storage) if write access isn't configured — this requires the account to
// have Editor access to the sheet, not just Viewer.
export async function writePurchaseInfo(sheetName, id, { purchaser, price, purchasedDate }) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth) {
  ...
  // Some tabs (e.g. a narrower NTS import) don't have columns out to AD:AF
  // yet — widen the grid first, or the write below fails with "exceeds grid
  // limits" instead of landing.
  const requiredCols = colLetterToIndex(DATE_COL);
