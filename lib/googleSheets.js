...
const HEADER_SCAN_RANGE = `A1:${FOLLOWUP_COL}3000`;
...
// Reads columns AE (purchaser), AF (price), and AG (purchase date) for each
...
export async function fetchPurchaseInfo(sheetName) {
  ...
  try {
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: `${sheetName}!${PURCHASER_COL}2:${DATE_COL}3000`,
    });
    const rows = res.data.values || [];
    return rows.map((row) => ({ purchaser: row[0] || "", price: row[1] || "", purchasedDate: row[2] || "" }));
  } catch (err) {
    // Columns AE:AG don't exist yet on a narrower tab (grid isn't wide
...
export async function diagnoseSheet(sheetName) {
  ...
  try {
    const valuesRes = await sheets.spreadsheets.values.get({
      spreadsheetId: GOOGLE_SHEET_ID,
      range: `${sheetName}!A1:${FOLLOWUP_COL}5`,
    });
    const values = valuesRes.data.values || [];
