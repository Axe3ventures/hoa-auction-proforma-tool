...
function classifyColor(bg) {
  ...
  // Generous thresholds so ANY visible shade counts — pale tints (light red/
  // green 3), dark shades (maroon, forest), and off-hues (rose, teal-leaning
  // greens) all classify rather than only the standard palette chips.
  if (s < 0.08 || l > 0.96 || l < 0.04) return "none"; // grayscale / white / near-black
  if (h >= 330 || h < 20) return "red";
  // Orange tops out at ~36° in the Sheets palette; yellows start at ~44°, so
  // cutting at 42° keeps every orange chip while excluding pale yellows.
  if (h >= 20 && h < 42) return "orange";
  if (h >= 42 && h < 65) return "yellow"; // Flyer status
  // Green stops at 170° — cyan/teal (~180°) and anything bluer intentionally
  // classify as "none" so blue-highlighted rows stay in the active list.
  if (h >= 65 && h < 170) return "green";
  if (h >= 288 && h < 330) return "magenta"; // Contact status (pink/magenta)
  // 170–288 (cyan / blue / purple) stays "none" so those rows remain active.
  return "none";
}
...
const HIGHLIGHT_COLORS = {
  ...
  red: { red: 0.878, green: 0.4, blue: 0.4 }, //   #e06666 "light red 1" — hue 0 (Canceled)
  yellow: { red: 1, green: 0.851, blue: 0.4 }, //   #ffd966 "yellow" — hue ~45 (Flyer)
  magenta: { red: 0.851, green: 0.447, blue: 0.753 }, // #d972c0 — hue ~314 (Contact)
...
export async function setRowColor(sheetName, id, colorName, sheetRow) {
  ...
  const rowIndex = locateRowIndex(rows, idColIndex, id, sheetRow);
...
const LOCK_COL = "AJ";
// "TRUE" once the user has pressed Fully Researched on the property — drives
// the 👍 badge on the property list. Empty means not (or no longer) marked.
const RESEARCHED_COL = "AK";
...
const HEADER_SCAN_RANGE = `A1:${RESEARCHED_COL}3000`;

// Finds the 0-indexed data-row position for a write, given the ID and (when
// the caller has it) the exact 1-indexed sheet row the property was read from.
// The sheet contains DUPLICATE IDs (25 of them as of 2026-07), so matching by
// ID alone can silently write to the wrong (older) occurrence — that's how
// "Lock Numbers" appeared not to stick (e.g. Rancho Vista, id 535, exists on
// rows 545 and 1272; the lock landed on 545). The sheetRow hint wins whenever
// the ID at that row still matches; otherwise fall back to the LAST id match,
// since duplicated IDs are old rows re-listed lower down with fresh data.
function locateRowIndex(rows, idColIndex, id, sheetRowHint) {
  if (sheetRowHint) {
    const idx = Number(sheetRowHint) - 2;
    if (idx >= 0 && idx < rows.length && String(rows[idx]?.[idColIndex]) === String(id)) return idx;
  }
  for (let i = rows.length - 1; i >= 0; i--) {
    if (String(rows[i]?.[idColIndex]) === String(id)) return i;
  }
  return -1;
}
...
// sale price), AJ (locked scenario JSON), and AK (fully-researched flag) for
// each data row (row 2 onward), aligned with the data rows the same way
// fetchRowColors is. Returns null if Sheets isn't configured.
export async function fetchPurchaseInfo(sheetName) {
  ...
  try {
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: `${sheetName}!${PURCHASER_COL}2:${RESEARCHED_COL}3000`,
    });
...
export async function writePurchaseInfo(sheetName, id, { purchaser, price, purchasedDate, followUpDate }, sheetRow) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth) {
  ...
  const targetRow = rowIndex + 2; // +1 for the header row, +1 for 1-indexing
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId: GOOGLE_SHEET_ID,
    requestBody: {
      valueInputOption: "RAW",
      data: [
        { range: `${sheetName}!${PURCHASER_COL}${targetRow}`, values: [[purchaser || ""]] },
        { range: `${sheetName}!${PRICE_COL}${targetRow}`, values: [[price || ""]] },
        { range: `${sheetName}!${DATE_COL}${targetRow}`, values: [[purchasedDate || ""]] },
        { range: `${sheetName}!${FOLLOWUP_COL}${targetRow}`, values: [[followUpDate || ""]] },
      ],
    },
  });
  return true;
...
// Writes just the price into column AF (Price Paid) for the row matching
// `id`, without touching purchaser/date/color — this is how the Purchase/Bid
// Price typed in the app's The Numbers section syncs to the sheet. A bare
// price in AF is a working bid number, NOT a purchase (see classifySale);
// marking a row purchased still goes through writePurchaseInfo. Returns false
// if write access isn't configured or the row can't be found.
export async function writeBidPrice(sheetName, id, price, sheetRow) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth || !GOOGLE_SHEET_ID) return false;
  ...
  const requiredCols = colLetterToIndex(PRICE_COL);
  const meta = await sheets.spreadsheets.get({ spreadsheetId: GOOGLE_SHEET_ID, fields: "sheets.properties" });
  const sheetProps = meta.data.sheets?.find((s) => s.properties.title === sheetName)?.properties;
  const currentCols = sheetProps?.gridProperties?.columnCount || 0;
  if (sheetProps && currentCols < requiredCols) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: GOOGLE_SHEET_ID,
    range: `${sheetName}!${PRICE_COL}${targetRow}`,
    valueInputOption: "RAW",
...
export async function writeFinalSalePrice(sheetName, id, finalSalePrice, sheetRow) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth || !GOOGLE_SHEET_ID) return false;
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: GOOGLE_SHEET_ID,
    range: `${sheetName}!${FINAL_SALE_COL}${targetRow}`,
    valueInputOption: "RAW",
...
export async function writeLockedScenario(sheetName, id, scenario, sheetRow) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth || !GOOGLE_SHEET_ID) return false;
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: GOOGLE_SHEET_ID,
    range: `${sheetName}!${LOCK_COL}${targetRow}`,
    valueInputOption: "RAW",
...
// Sets (or clears) the fully-researched flag in column AK for the row
// matching `id` — persisted in the sheet so the 👍 badge survives refreshes
// and shows on every device. Returns false if write access isn't configured
// or the row can't be found.
export async function writeResearched(sheetName, id, researched, sheetRow) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth || !GOOGLE_SHEET_ID) return false;
  ...
  const requiredCols = colLetterToIndex(RESEARCHED_COL);
  const meta = await sheets.spreadsheets.get({ spreadsheetId: GOOGLE_SHEET_ID, fields: "sheets.properties" });
  const sheetProps = meta.data.sheets?.find((s) => s.properties.title === sheetName)?.properties;
  const currentCols = sheetProps?.gridProperties?.columnCount || 0;
  if (sheetProps && currentCols < requiredCols) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: GOOGLE_SHEET_ID,
    range: `${sheetName}!${RESEARCHED_COL}${targetRow}`,
    valueInputOption: "RAW",
    requestBody: { values: [[researched ? "TRUE" : ""]] },
  });
  return true;
}
...
export async function appendDriveByNote(sheetName, id, noteText, sheetRow) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth || !GOOGLE_SHEET_ID) return false;
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: GOOGLE_SHEET_ID,
    range: `${sheetName}!${DRIVE_BY_NOTE_COL}${targetRow}`,
    valueInputOption: "RAW",
...
export async function clearDriveByNotes(sheetName, id, sheetRow) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth || !GOOGLE_SHEET_ID) return false;
