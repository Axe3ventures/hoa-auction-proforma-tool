...
// Purchaser name and sale price live at these FIXED columns on every sheet
// tab, independent of header text — deliberately not looked up by header
// name (unlike the ID column) so this doesn't depend on any particular
// header label existing at all.
...
// AD/AE are the actual first empty columns, confirmed empty across all rows
// before use.
const PURCHASER_COL = "AD";
const PRICE_COL = "AE";

// Reads columns AB (purchaser) and AC (price) for each data row (row 2
// onward), aligned with the data rows the same way fetchRowColors is. Returns
// null if Sheets isn't configured.
export async function fetchPurchaseInfo(sheetName) {
  ...
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: `${sheetName}!${PURCHASER_COL}2:${PRICE_COL}1000`,
  });

  const rows = res.data.values || [];
  return rows.map((row) => ({ purchaser: row[0] || "", price: row[1] || "" }));
}

// Writes the purchaser name and sale price into columns AB/AC for the row
// matching `id` (still looked up by the ID column, wherever that is), so
// purchase details survive across machines/deploys instead of living only in
// a local file. Pass empty strings to clear both (undoing a purchase).
// Returns false (caller should fall back to local storage) if write access
// isn't configured — this requires the account to have Editor access to the
// sheet, not just Viewer.
export async function writePurchaseInfo(sheetName, id, { purchaser, price }) {
  const { GOOGLE_SHEET_ID } = process.env;
  const auth = getAuth(["https://www.googleapis.com/auth/spreadsheets"]);
  if (!auth) {
