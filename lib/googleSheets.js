...
const HEADER_SCAN_RANGE = `A1:${FINAL_SALE_COL}3000`;
...
// Reads columns AE (purchaser), AF (price), AG (purchase date), and AI (final
// sale price) for each data row (row 2 onward), aligned with the data rows
// the same way fetchRowColors is. Returns null if Sheets isn't configured.
export async function fetchPurchaseInfo(sheetName) {
  ...
  try {
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: `${sheetName}!${PURCHASER_COL}2:${FINAL_SALE_COL}3000`,
    });
...
export async function appendDriveByNote(sheetName, id, noteText) {
  ...
  const stamped = `[${new Date().toLocaleString("en-US")}] ${noteText}`;
