...
import { READ_CELL_RANGE } from "./sheetConfig";
...
async function getEliminationStatus(sourceKey) {
  ...
  const rows = await fetchSheetRows(`${sheetName}!${READ_CELL_RANGE}`);
  ...
  rows
    // Original index preserved so colors[i]/purchaseInfo[i] (aligned to the
    // full sheet, blank-ID rows included) stay in sync — see normalize() in
    // the properties route for why filtering first misaligns them.
    .map((r, i) => ({ r, i }))
    .filter(({ r }) => r.ID)
    .forEach(({ r, i }) => {
